...
# 
# Usage:
#   ./dev-server.sh start    - Start OpenCode + Next.js in tmux session with windows
#   ./dev-server.sh stop     - Stop all services
#   ./dev-server.sh status   - Check if services are running
#   ./dev-server.sh attach   - Attach to tmux session
...
cmd_start() {
    ...
    echo "  0: OpenCode Server (Port 9090)"
    ...
    echo ""
     ...
     echo "  Dev State:    ./dev/app/.local/"
     echo "  Dev Data:     ./data/projects/"
    echo ""
...
case "${1:-}" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  ${GREEN}start${NC}   - Start OpenCode + Next.js (windows in one tmux session)"
